...
Usage:
    ...
    convert_to_pdf("presentation.pptx", "/tmp/output")
"""
...
def convert_to_pdf(input_path: str, output_dir: str) -> Path:
    """Convert a PPTX file to PDF using Microsoft PowerPoint via AppleScript."""
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"PowerPoint PDF conversion failed: {result.stderr.strip()}"
        )
...
def run_office_convert(input_path: str, output_dir: str, fmt: str = "pdf") -> Path:
    ...
    Supports:
        pdf  - PPTX to PDF via PowerPoint
    """
    if fmt == "pdf":
